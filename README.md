...
Synthetic image generation via compositing and/or inpainting.
...
Two providers satisfy the `InpaintingProvider` protocol:

```python
# Google Imagen 3 (API-based, default)
from synthdet.generate.providers.imagen import ImagenInpaintingProvider

provider = ImagenInpaintingProvider(
    api_key="...",           # or set GOOGLE_API_KEY env var
    model="imagen-3.0-capability-001",
    guidance_scale=75.0,
)

# Local Stable Diffusion (requires GPU + diffusers)
...
Zero-shot object detection for annotating images without ground-truth labels (e.g., web-scraped images). Two annotators satisfy the `Annotator` protocol.
...
| `AnnotationSource` | Enum: `human`, `compositor`, `inpainting`, `grounding_dino`, `owl_vit`, `sam_refined`. |
...
| `api` | google-genai | Google Imagen 3 inpainting |
